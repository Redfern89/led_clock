...
int main(void) {
	...
	// Таймер для подсчета миллисекунд
...
//	};
	...
	while (1) {
		// Рапределение значения по массиву
		// Так, как я дико объебался с распределением диодов по шкале, лепим костыли
		display_data[5] = ((((scale >> 4) & 0b00011111) << 3) | (scale >> 61)); // Берем первые 5 бит и последние 3 бита и клеим в один байт
		...
		display_data[4] = (unsigned char)((scale >> 9) << 4) | 0b00001111; // Последняя группа ваще из 4х светодиодов, дначит отключаем все, что после 4х
